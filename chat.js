...
//  stopButton.innerHTML = 'Stop';
  ...
  const cl_stop_chat = document.querySelector(".cl_stop_chat");
 // sendButton.insertAdjacentElement('beforebegin', stopButton);
  cl_stop_chat.append(stopButton);
...
function deleteChat() {
  ...
  localStorage.removeItem('chat_name'+selectedChat);
  updateChatList();
}
...
function loadSelectedChat() {
  const selectedChat = document.getElementById("chat-select").value;
  const obj = JSON.parse(localStorage.getItem('chat_name'+selectedChat));
...
function updateChatList() {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    if (key === "host-address" || key.search(/chat_name/) !== 0) continue;
    key = key.replace("chat_name","");
    const option = document.createElement("option");
    option.value = key;
    option.text = key;
    chatList.add(option);
